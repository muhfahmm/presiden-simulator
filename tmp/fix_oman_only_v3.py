...
# Unify with Iran Kanan logic:
# 1) Converge to 59.80, 23.00 to avoid crossing Suez
# 2) For Asia, jump to 63.50, 21.00 (which is where Iran Kanan joins).
...
for dest, coords in routes_data["Kuwait"].items():
    ...
    if dest == "Terusan Suez":
        ...
        idx_connect = find_nearest_index(coords, 63.50, 21.00)
        sliced_coords = [{'lon': 58.59, 'lat': 23.58}, {'lon': 59.80, 'lat': 23.00}] + coords[idx_connect:]
