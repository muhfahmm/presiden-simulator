...
yemen = get_coords(content, "Yemen (Aden)")
kuwait = get_coords(content, "Kuwait")

destinations = list(kuwait.keys())
...
for dest, coords in kuwait.items():
    ...
    # Merge at 52.50, 26.30 (Same as Qatar logic)
    best_i = -1
...
# We need the Yemen Aden paths. If Yemen Aden doesn't have a destination, we can use India (Mumbai) up to 72.85, 19.00 and append Kuwait's path from there!
# Notice Kuwait routes to Asia ALL pass through 72.85, 19.00 !!
def build_red_sea_asia(start_point, intermediate_points):
    res = {}
    for dest, coords in kuwait.items():
        if dest == "Terusan Suez": continue
        
        # Find 72.85, 19.00 in Kuwait
        k_idx = -1
        for i, c in enumerate(coords):
...
with open(r"c:\fhm\EM4\tmp\inject_saudi.py", 'w', encoding='utf-8') as f:
    f.write("""import json
with open(r"c:\\fhm\\EM4\\app\\frontend-desktop\\src\\app\\game\\trades\\international\\routes.ts", 'w', encoding='utf-8') as f:
    f.write(r'''""" + new_content.replace('\\', '\\\\') + """''')
print("Injected Saudi")
""")
