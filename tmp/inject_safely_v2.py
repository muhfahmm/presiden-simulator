...
if "Kuwait" in routes_data:
    ...
    for dest, coords in routes_data["Kuwait"].items():
        if coords:
            # UAE target coordinate { lon: 54.00, lat: 25.50 }
            idx_connect = find_nearest_index(coords, 54.00, 25.50)
            if idx_connect != -1:
                sliced_coords = [{'lon': 54.30, 'lat': 24.40}] + coords[idx_connect:]
                output_ts += f'    "{dest}": {{\n'
...
if "Kuwait" in routes_data:
    ...
    for dest, coords in routes_data["Kuwait"].items():
        if coords:
            # BIAS TARGET EAST: { lon: 59.50, lat: 23.50 } to connect downstream
            idx_connect = find_nearest_index(coords, 59.50, 23.50)
            if idx_connect != -1:
