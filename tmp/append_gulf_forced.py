...
# 3. Apply Forced Snap for Bushehr
suborigins_gulf = {
    "Iran (Bushehr)": { "lon": 50.84, "lat": 28.97, "forced_join": 2 }, # Snap list after 51.00
...
for name, hub_coord in suborigins_gulf.items():
    ...
    for dname, dinfo in kuwait_dests.items():
        ...
        if "forced_join" in hub_coord:
            # Force Index
            best_idx = hub_coord["forced_join"]
        else:
            min_distance = 9999.0
            best_idx = 0
        ...
        # Slice forward FROM the selected index
...
print("Gulf slices updated with forced index.")
