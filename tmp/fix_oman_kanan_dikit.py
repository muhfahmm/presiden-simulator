...
for dest, coords in routes_data["Kuwait"].items():
    ...
    # User wanted Oman to merge further right ("kekanan dikit" / downstream).
    # Since the trunk skips `{ 59.00, 24.10 }` for some Asia routes, 
    # we explicitly inject this point, then attach downstream securely avoiding crossings.
    
    if dest == "Terusan Suez":
        # Downstream for Suez is 59.80, 23.00
        idx_connect = find_nearest_index(coords, 59.80, 23.00)
    else:
        # Downstream for Asia
        idx_connect = find_nearest_index(coords, 60.00, 23.50)
        
    if idx_connect != -1:
        sliced_coords = [{'lon': 58.59, 'lat': 23.58}, {'lon': 59.00, 'lat': 24.10}] + coords[idx_connect:]
        output_ts += f'    "{dest}": {{\n'
