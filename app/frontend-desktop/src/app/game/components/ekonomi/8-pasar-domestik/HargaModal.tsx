...
import { X, Tag, ShoppingCart, Zap, Fuel, Pill, GraduationCap, AlertCircle, TrendingDown, TrendingUp, Droplets, Wheat, Beef, Droplet, Package, Egg } from "lucide-react";
        ...
        <div className="flex-1 overflow-y-auto p-12 custom-scrollbar space-y-12 relative z-10 bg-[radial-gradient(circle_at_top_right,rgba(6,182,212,0.03),transparent_40%)]">
          ...
          <div className="flex flex-col gap-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="p-8 bg-zinc-900/40 border border-white/5 rounded-[2rem] relative overflow-hidden group hover:bg-zinc-900/60 transition-all shadow-lg">
                 <div className={`absolute top-0 left-0 w-1.5 h-full ${happinessImpact >= 0 ? "bg-emerald-500" : "bg-rose-500"}`} />
                 <p className="text-zinc-500 text-[10px] font-black uppercase tracking-widest mb-2">Dampak Kepuasan</p>
                 <div className="flex items-end gap-2">
                   <span className={`text-4xl font-black italic ${happinessImpact >= 0 ? "text-emerald-400" : "text-rose-400"}`}>
                     {happinessImpact >= 0 ? "+" : ""}{happinessImpact.toFixed(1)}%
                   </span>
                   <div className="pb-1 text-zinc-600 font-bold text-xs uppercase tracking-tighter">Per Bulan</div>
                 </div>
              </div>

              <div className="p-8 bg-zinc-900/40 border border-white/5 rounded-[2rem] relative overflow-hidden group hover:bg-zinc-900/60 transition-all shadow-lg">
                 <p className="text-zinc-500 text-[10px] font-black uppercase tracking-widest mb-2">Price Index</p>
               <div className="flex items-end gap-2">
                 <span className="text-4xl font-black italic text-zinc-100">{(avgPriceMultiplier * 100).toFixed(0)}%</span>
                 <div className="pb-1 text-zinc-600 font-bold text-xs uppercase tracking-tighter">National Avg</div>
               </div>
              </div>

              <div className="p-8 bg-zinc-900/40 border border-white/5 rounded-[2rem] relative overflow-hidden group hover:bg-zinc-900/60 transition-all shadow-lg">
                 <p className="text-zinc-500 text-[10px] font-black uppercase tracking-widest mb-2">Total Harga Pasar</p>
                 <div className="flex items-end gap-2">
                   <span className="text-4xl font-black italic text-cyan-400">{formatPrice(totalCartPrice)}</span>
                   <div className="pb-1 text-zinc-600 font-bold text-xs uppercase tracking-tighter">{totalItems} Komoditas</div>
                 </div>
              </div>
            </div>
