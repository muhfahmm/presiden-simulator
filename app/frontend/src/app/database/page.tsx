            ...
            <div className={`transition-all duration-300 ease-in-out overflow-hidden ${isSocialOpen ? "max-h-[500px] opacity-100 mt-2" : "max-h-0 opacity-0 pointer-events-none"}`}>
              <div className="flex flex-col gap-4 overflow-y-auto max-h-[500px] no-scrollbar pr-1">
                <div className="flex items-center justify-between text-xs font-black bg-zinc-800/50 p-2 rounded-xl border border-zinc-700/30 mt-1 shadow-inner">
                  <span className="text-zinc-400 flex items-center gap-1"><Zap size={10} className="text-amber-500" /> Beban Listrik</span>
                  <span className="text-amber-500 text-sm">
                    {hitungKonsumsiSosial(currentData).toLocaleString('id-ID')} MW
                    <span className="text-zinc-500 text-xs font-bold font-sans ml-1">({(currentData.pendidikan?.prasekolah || 0) + (currentData.pendidikan?.dasar || 0) + (currentData.pendidikan?.menengah || 0) + (currentData.pendidikan?.lanjutan || 0) + (currentData.pendidikan?.universitas || 0) + (currentData.pendidikan?.lembaga_pendidikan || 0) + (currentData.pendidikan?.laboratorium || 0) + (currentData.pendidikan?.observatorium || 0) + (currentData.pendidikan?.pusat_penelitian || 0) + (currentData.pendidikan?.pusat_pengembangan || 0) + (currentData.kesehatan?.rumah_sakit_besar || 0) + (currentData.kesehatan?.rumah_sakit_kecil || 0) + (currentData.kesehatan?.pusat_diagnostik || 0)} Unit)</span>
                  </span>
                </div>
                {/* Education & Research */}
                <div className="flex flex-col gap-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-black text-blue-400">{currentData.pendidikan?.literasi || 0}% LT</span>
                  </div>
                  <div className={`grid ${getGridCols(rightWidth)} gap-2 mt-1`}>
                    <SectorStat icon={<Building2 size={10} className="text-zinc-400" />} label="Pra/Dasar" value={`${(currentData.pendidikan?.prasekolah || 0) + (currentData.pendidikan?.dasar || 0)} (${((currentData.pendidikan?.prasekolah || 0) * KONSUMSI_SOSIAL.pendidikan.prasekolah + (currentData.pendidikan?.dasar || 0) * KONSUMSI_SOSIAL.pendidikan.dasar).toLocaleString('id-ID')} MW)`} />
                    <SectorStat icon={<Library size={10} className="text-zinc-300" />} label="Men/Lanjut" value={`${(currentData.pendidikan?.menengah || 0) + (currentData.pendidikan?.lanjutan || 0)} (${((currentData.pendidikan?.menengah || 0) * KONSUMSI_SOSIAL.pendidikan.menengah + (currentData.pendidikan?.lanjutan || 0) * KONSUMSI_SOSIAL.pendidikan.lanjutan).toLocaleString('id-ID')} MW)`} />
                    <SectorStat icon={<Library size={10} className="text-zinc-200" />} label="PT/Lembaga" value={`${(currentData.pendidikan?.universitas || 0) + (currentData.pendidikan?.lembaga_pendidikan || 0)} (${((currentData.pendidikan?.universitas || 0) * KONSUMSI_SOSIAL.pendidikan.universitas + (currentData.pendidikan?.lembaga_pendidikan || 0) * KONSUMSI_SOSIAL.pendidikan.lembaga_pendidikan).toLocaleString('id-ID')} MW)`} />
                    <SectorStat icon={<Microscope size={10} className="text-emerald-400" />} label="Lab & Riset" value={`${(currentData.pendidikan?.laboratorium || 0) + (currentData.pendidikan?.pusat_penelitian || 0)} (${((currentData.pendidikan?.laboratorium || 0) * KONSUMSI_SOSIAL.pendidikan.laboratorium + (currentData.pendidikan?.pusat_penelitian || 0) * KONSUMSI_SOSIAL.pendidikan.pusat_penelitian).toLocaleString('id-ID')} MW)`} />
                    <SectorStat icon={<Eye size={10} className="text-purple-300" />} label="Observatorium" value={`${currentData.pendidikan?.observatorium || 0} (${((currentData.pendidikan?.observatorium || 0) * KONSUMSI_SOSIAL.pendidikan.observatorium).toLocaleString('id-ID')} MW)`} />
                    <SectorStat icon={<Lightbulb size={10} className="text-yellow-400" />} label="Pengembangan" value={`${currentData.pendidikan?.pusat_pengembangan || 0} (${((currentData.pendidikan?.pusat_pengembangan || 0) * KONSUMSI_SOSIAL.pendidikan.pusat_pengembangan).toLocaleString('id-ID')} MW)`} />
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-black text-emerald-400">{currentData.kesehatan?.indeks_kesehatan || 0}% IX</span>
                  </div>
                  <div className={`grid ${getGridCols(rightWidth)} gap-2 mt-1`}>
                    <SectorStat icon={<Building2 size={10} className="text-emerald-500" />} label="RS Besar/Kecil" value={`${(currentData.kesehatan?.rumah_sakit_besar || 0) + (currentData.kesehatan?.rumah_sakit_kecil || 0)} (${((currentData.kesehatan?.rumah_sakit_besar || 0) * KONSUMSI_SOSIAL.kesehatan.rumah_sakit_besar + (currentData.kesehatan?.rumah_sakit_kecil || 0) * KONSUMSI_SOSIAL.kesehatan.rumah_sakit_kecil).toLocaleString('id-ID')} MW)`} />
                    <SectorStat icon={<Search size={10} className="text-cyan-400" />} label="Diagnostik" value={`${currentData.kesehatan?.pusat_diagnostik || 0} (${((currentData.kesehatan?.pusat_diagnostik || 0) * KONSUMSI_SOSIAL.kesehatan.pusat_diagnostik).toLocaleString('id-ID')} MW)`} />
                    <SectorStat icon={<Beef size={10} className="text-red-400" />} label="Harapan Hidup" value={currentData.kesehatan?.harapan_hidup || 0} />
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-black text-orange-400">{currentData.hukum?.indeks_keamanan || 0}% SEC</span>
                  </div>
                  <div className={`grid ${getGridCols(rightWidth)} gap-2 mt-1`}>
                    <SectorStat icon={<GraduationCap size={10} className="text-zinc-200" />} label="Akademi Polisi" value={`${currentData.hukum?.akademi_polisi || 0} (${((currentData.hukum?.akademi_polisi || 0) * KONSUMSI_SOSIAL.hukum.akademi_polisi).toLocaleString('id-ID')} MW)`} />
                    <SectorStat icon={<ShieldAlert size={10} className="text-blue-500" />} label="Kepolisian" value={`${currentData.hukum?.pos_polisi || 0} (${((currentData.hukum?.pos_polisi || 0) * KONSUMSI_SOSIAL.hukum.pos_polisi).toLocaleString('id-ID')} MW)`} />
                    <SectorStat icon={<Car size={10} className="text-zinc-400" />} label="Armada Mobil" value={`${currentData.hukum?.armada_mobil_polisi || 0} (${((currentData.hukum?.armada_mobil_polisi || 0) * KONSUMSI_SOSIAL.hukum.armada_mobil_polisi).toLocaleString('id-ID')} MW)`} />
                    <SectorStat icon={<Gavel size={10} className="text-orange-400" />} label="Kejaksaan/Court" value={`${(currentData.hukum?.kejaksaan || 0) + (currentData.hukum?.pengadilan || 0)} (${((currentData.hukum?.kejaksaan || 0) * KONSUMSI_SOSIAL.hukum.kejaksaan + (currentData.hukum?.pengadilan || 0) * KONSUMSI_SOSIAL.hukum.pengadilan).toLocaleString('id-ID')} MW)`} />
                    <SectorStat icon={<Scale size={10} className="text-yellow-500" />} label="Bantuan Hukum" value={`${currentData.hukum?.pusat_bantuan_hukum || 0} (${((currentData.hukum?.pusat_bantuan_hukum || 0) * KONSUMSI_SOSIAL.hukum.pusat_bantuan_hukum).toLocaleString('id-ID')} MW)`} />
                    <SectorStat icon={<TrendingUp size={10} className="text-zinc-400" />} label="Indeks Korupsi" value={currentData.hukum?.indeks_korupsi || 0} />
                  </div>
                </div>
              </div>
            </div>
