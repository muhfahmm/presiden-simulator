...
export function useGamePath(path: string[]) {
  ...
  if (countryByPath) {
    ...
    let filter = path[1] || "all";
    if (filter === "nuklir") filter = "program_nuklir";
  ...
  React.useEffect(() => {
    ...
    if (!targetPath && activeMenu.startsWith("CountryModal:")) {
      ...
      if (subTab) {
        targetPath += `/${subTab}`;
        if (subTab === "detail_lengkap") {
          if (sector && sector !== tab) targetPath += `/${sector}`;
          else if (!sector) targetPath += `/produksi`;
    ...
    if (!targetPath && activeMenu.startsWith("laporan_")) {
      ...
      if (sub === "war_page") targetPath += "/war_page";
    }
  ...
  React.useEffect(() => {
    ...
    const handlePopState = () => {
        // Silent sync - the app will handle it via params if needed
    };
