...
export default function GamePageClient() {
  ...
  useEffect(() => {
    ...
    const loadVotingTimer = async () => {
      if (userCountry) {
        const { initVotingTimer } = await import("../components/2_navigasi_menu/2_navigasi_bawah/5_geopolitik/1_PBB/1_pemungutan_suara/logika_pemungutan_suara/pengembangan_waktu/votingTimerLogic");
  ...
  const handleSelectSDA = React.useCallback((data: { name: string; flag: string; resources: any } | null) => {
