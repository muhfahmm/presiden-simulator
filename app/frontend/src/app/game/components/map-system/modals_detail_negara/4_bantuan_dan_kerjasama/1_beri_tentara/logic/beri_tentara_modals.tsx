...
export default function ModalsBeriTentara({
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      if (pRegistry) {
        try {
          const mod = await import(`@/app/database/data/semua_fitur_negara/2_militer/2_armada_militer/${pRegistry.continent}/${pRegistry.id}_${(playerCountry || "indonesia").toLowerCase().replace(/ /g, '_')}`);
      ...
      if (tRegistry) {
        try {
          const mod = await import(`@/app/database/data/semua_fitur_negara/2_militer/2_armada_militer/${tRegistry.continent}/${tRegistry.id}_${targetCountry.toLowerCase().replace(/ /g, '_')}`);
