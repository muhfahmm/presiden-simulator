...
interface MERCOSURProps {
  currentCash: number;
  currentDate: string;
  onUpdate: () => void;
}

export default function MERCOSUR({ currentCash, currentDate, onUpdate }: MERCOSURProps) {
  const userCountryName = (localStorage.getItem("selectedCountry") || "Indonesia").toLowerCase();
  const isEligible = unMembershipStorage.isEligibleForRegional("mercosur", userCountryName);

  const handleJoin = () => {
    if (!isEligible) {
      alert("Permohonan Ditolak: MERCOSUR hanya menerima negara anggota dari wilayah Amerika Selatan.");
      return;
    }

    unMembershipStorage.joinOrganization("mercosur");
    onUpdate();
  };
      ...
      <div className="bg-gradient-to-br from-purple-600/10 to-zinc-900/40 border border-purple-500/10 rounded-[32px] p-8 relative overflow-hidden group">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h3 className="text-2xl font-black text-white uppercase italic tracking-tight">MERCOSUR</h3>
              <p className="text-[10px] text-purple-400 font-bold uppercase tracking-[0.2em]">MERCOSUR</p>
            </div>
          </div>
          <p className="text-sm text-zinc-400 leading-relaxed max-w-2xl font-medium">
            Blok perdagangan dan integrasi ekonomi di Amerika Selatan.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-zinc-900/30 border border-zinc-800 rounded-2xl p-6 flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-lg font-black text-white italic">+20% Ekon. Amerika Latin</p>
          </div>
        </div>
        <div className="bg-zinc-900/30 border border-zinc-800 rounded-2xl p-6 flex items-center gap-6">
