...
import { Plane, Coins, Zap, CheckCircle2 } from "lucide-react";
import { unICAOStorage } from "./unICAOStorage";
...
export default function ICAOMenu({ currentCash, currentDate, onUpdate }: OrgProps) {
    const [state, setState] = useState(unICAOStorage.getData());

    useEffect(() => {
        setState(unICAOStorage.getData());
    }, []);

    const handleJoin = () => {
        const result = unICAOStorage.join(currentCash, currentDate);
        if (result.success) {
            setState(unICAOStorage.getData());
    ...
    return (
        <div className="bg-zinc-950/20 border border-zinc-800/30 rounded-3xl p-6 space-y-6 backdrop-blur-md">
            <div className="flex items-center gap-3">
                <div className="p-2.5 bg-zinc-500/10 rounded-xl border border-zinc-500/20">
                    <Plane className="h-5 w-5 text-zinc-400" />
                </div>
                <h3 className="text-lg font-black text-white uppercase italic">ICAO Details</h3>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4 text-left font-sans not-italic">
                <div className="p-4 bg-zinc-900/40 border border-zinc-800/50 rounded-2xl">
                    <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-1.5 flex items-center gap-2">
                        <Zap size={12} className="text-zinc-400" /> Dampak Bergabung
                    </p>
                    <p className="text-[11px] text-zinc-300 font-bold leading-relaxed">
                        Pajak Bandara +10% & Jalur Udara Efisien.
                    </p>
                </div>
