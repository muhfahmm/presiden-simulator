...
interface GantiIdeologiModalsProps {
  ideology: string;
  cost: number;
  onClose: () => void;
  onConfirm: () => void;
...
export function GantiIdeologiModals({ ideology, cost, onClose, onConfirm }: GantiIdeologiModalsProps) {
  ...
  return (
    <div className="absolute inset-0 z-30 bg-zinc-950/98 backdrop-blur-3xl flex flex-col p-6 animate-in fade-in zoom-in-95 duration-300 border border-white/5 rounded-[32px]">
      {/* Header */}
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center gap-2">
          ...
          <h4 className="text-[10px] font-black text-white uppercase tracking-widest italic font-sans">Konfirmasi Ideologi</h4>
        </div>
      ...
      <div className="flex-1 flex flex-col justify-center text-center px-2">
        <p className="text-[13px] font-bold text-zinc-300 leading-relaxed tracking-tight font-sans">
          Apakah Anda yakin ingin menetapkan <span className="text-cyan-400 italic uppercase">"{ideology}"</span> sebagai haluan resmi negara?
        </p>
        <div className="mt-4 p-3 bg-white/5 rounded-2xl border border-white/5 space-y-3">
          <p className="text-[10px] text-zinc-500 font-medium leading-normal italic font-sans">
            "Pergeseran ideologi nasional akan mengubah struktur sosial dan fundamental ekonomi secara permanen."
          </p>
          <div className={`flex items-center justify-between p-2.5 rounded-xl border ${isInsufficient ? 'bg-rose-500/10 border-rose-500/30' : 'bg-cyan-500/10 border-cyan-500/30'}`}>
            <div className="flex items-center gap-2">
