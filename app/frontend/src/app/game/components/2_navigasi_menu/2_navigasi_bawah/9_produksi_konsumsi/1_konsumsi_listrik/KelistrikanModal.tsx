...
import { X, Bolt, Activity, TrendingUp, TrendingDown, Battery, Radio, Gauge, Info, Hammer, Shield, Users, Factory, Pickaxe, Lightbulb, ZapOff, CloudLightning, Sprout, Utensils, Pill, MapPin, Bus, TrainFront, Plane, Ship, School, Hospital, Gavel, Trophy, Store, Theater, Home, Truck, Eye, EyeOff } from "lucide-react";
import { 
...
export default function KelistrikanModal({ isOpen, onClose, setActiveMenu }: KelistrikanModalProps) {
  ...
  const produksiUsage = [
    { name: "Sektor Ekstraksi Mineral", value: hitungKonsumsiEkstraksi(currentDataWithDeltas.sektor_ekstraksi), icon: Pickaxe, color: "text-orange-500", description: "Tambang & Smelter" },
    { name: "Sektor Industri Manufaktur", value: hitungKonsumsiProduksi(currentDataWithDeltas.sektor_manufaktur), icon: Factory, color: "text-emerald-500", description: "Pabrik Garmen, Kimia, dll" },
    { name: "Sektor Agrikultur & Ternak", value: hitungKonsumsiAgrikultur(currentDataWithDeltas.sektor_agrikultur) + hitungKonsumsiPeternakan(currentDataWithDeltas.sektor_peternakan), icon: Sprout, color: "text-lime-500", description: "Sawah, Ladang & Peternakan" },
    { name: "Sektor Olahan Pangan & Lab", value: hitungKonsumsiOlahanPangan(currentDataWithDeltas.sektor_olahan_pangan) + hitungKonsumsiFarmasi(currentDataWithDeltas.sektor_farmasi), icon: Utensils, color: "text-amber-500", description: "Industri Makanan & Farmasi" },
  ];

  const militerUsage = [
    { name: "Pabrik Militer", value: hitungKonsumsiPabrikMiliter(currentDataWithDeltas.pabrik_militer), icon: Shield, color: "text-rose-500", description: "Produksi Alutsista" },
  ];
  ...
  const armadaUsage = [
    {
      name: "Armada Militer Nasional",
      value: Object.entries(armadaMiliterRate).reduce((acc, [k, v]: [string, any]) => {
        const count = v.groupId === 'darat' ? (v.dataKey === 'barak' ? (currentDataWithDeltas.armada_militer.barak || 0) : ((currentDataWithDeltas.armada_militer.darat as any)[v.dataKey] || 0)) : ((currentDataWithDeltas.armada_militer[v.groupId] as any)[v.dataKey] || 0);
        return acc + count * (v.consumption || 0);
      }, 0),
      icon: Truck,
      color: "text-rose-500",
      description: "Darat, Laut & Udara"
    }
  ];
                    ...
                    <div key={idx} className="bg-zinc-950/50 border border-zinc-800/50 p-4 rounded-2xl flex items-center justify-between hover:border-zinc-700 transition-all group">
                      ...
                      <div className="text-right">
                        ...
                        <div className="h-1 w-20 bg-zinc-900 rounded-full mt-2 overflow-hidden">
                          <div className="h-full bg-rose-500/60" style={{ width: `${Math.min(100, (usage.value / (totalBeban || 1)) * 100)}%` }} />
                        </div>
                      </div>
                    </div>
                  ))}
