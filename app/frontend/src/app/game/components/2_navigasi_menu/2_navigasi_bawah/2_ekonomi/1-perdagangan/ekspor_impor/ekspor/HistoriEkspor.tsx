...
export const HistoriEkspor: React.FC = () => {
  ...
  const handleClear = () => {
    if (confirm("Hapus semua histori transaksi ekspor?")) {
  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="bg-zinc-900/40 border border-zinc-800/50 p-6 rounded-[2rem] flex items-center justify-between group overflow-hidden relative mb-8">
        <div className="absolute -right-4 -bottom-4 opacity-5 group-hover:scale-110 transition-transform duration-700 text-green-500">
          <TrendingUp size={100} />
        </div>
        <div className="relative z-10">
          <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-1">Total Transaksi Ekspor</p>
          <h4 className="text-3xl font-black text-white italic tracking-tighter">{history.length}</h4>
        </div>
        <div className="w-12 h-12 rounded-2xl bg-green-500/10 border border-green-500/20 flex items-center justify-center relative z-10">
          <TrendingUp size={24} className="text-green-500" />
        </div>
      </div>

      <div className="flex items-center justify-between mb-6">
        <h5 className="text-[11px] font-black uppercase tracking-[0.2em] text-zinc-400 italic">Daftar Histori Ekspor</h5>
        {history.length > 0 && (
          <button 
            onClick={handleClear}
