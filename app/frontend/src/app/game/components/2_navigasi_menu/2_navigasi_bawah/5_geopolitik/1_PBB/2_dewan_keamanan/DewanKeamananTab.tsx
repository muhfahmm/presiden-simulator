...
import { Globe, Crown, Users, Info, Calendar } from "lucide-react";
...
export default function DewanKeamananTab() {
  ...
  // Pre-calculate country map for faster lookups
  const countryMap = useMemo(() => {
    const map = new Map<string, string>();
    countries.forEach(c => {
      map.set(c.name_id.toLowerCase(), c.flag);
      map.set(c.name_en.toLowerCase(), c.flag);
    });
    return map;
  }, []);
  ...
  // Enriched members with ISO codes calculated only once
  const processedMembers = useMemo(() => {
    return members.map(m => {
      const emoji = m.flag || countryMap.get(m.name.toLowerCase()) || "";
      let iso = "";
  ...
  const { permanentMembers, nonPermanentMembers, newlyElected } = useMemo(() => {
    return {
      permanentMembers: processedMembers.filter(m => m.isPermanent),
      nonPermanentMembers: processedMembers.filter(m => !m.isPermanent && m.termStart <= gameYear),
      newlyElected: processedMembers.filter(m => !m.isPermanent && m.termStart > gameYear)
    };
  }, [processedMembers, gameYear]);
          ...
          <div className="p-4 flex flex-col gap-3">
            {permanentMembers.map((m, i) => (
              <div key={i} className="flex items-center gap-4 px-5 py-4 bg-zinc-900/60 border border-zinc-800/50 rounded-2xl group hover:border-amber-500/30 transition-all">
                <div className="flex items-center justify-center w-12 h-8 rounded-md bg-zinc-950 border border-zinc-800 overflow-hidden shadow-inner group-hover:border-amber-500/50 transition-colors">
                  {m.iso ? (
                    <img 
                      src={`https://flagcdn.com/w80/${m.iso}.png`} 
                      alt={m.name} 
                      className="w-full h-full object-cover" 
                    />
                  ) : (
                    <span className="text-xl">{m.flag || "🏳️"}</span>
                  )}
                </div>
          ...
          <div className="p-4 flex flex-col gap-2.5">
            ...
            {nonPermanentMembers.map((m, i) => (
              <div key={i} className="flex items-center gap-4 px-5 py-3.5 bg-zinc-900/60 border border-zinc-800/50 rounded-2xl group hover:border-sky-500/30 transition-all relative">
                <div className="flex items-center justify-center w-12 h-8 rounded-md bg-zinc-950 border border-zinc-800 overflow-hidden shadow-inner group-hover:border-sky-500/50 transition-colors">
                  {m.iso ? (
                    <img 
                      src={`https://flagcdn.com/w80/${m.iso}.png`} 
                      alt={m.name} 
            ...
            {newlyElected.map((m, i) => (
              <div key={`new-${i}`} className="flex items-center gap-4 px-5 py-3 border border-zinc-800/50 rounded-2xl group opacity-60 bg-zinc-950/20">
                <div className="flex items-center justify-center w-12 h-8 rounded-md bg-zinc-950 border border-zinc-800 overflow-hidden shadow-inner opacity-50">
                  {m.iso ? (
                    <img 
                      src={`https://flagcdn.com/w80/${m.iso}.png`} 
                      alt={m.name} 
                    ...
                    <span className="text-xl grayscale">{m.flag || "🏳️"}</span>
                  )}
                </div>
                <div className="flex-1">
