import { FileText, CheckCircle2, Clock, Globe, Target, XCircle } from "lucide-react";
...
export function ActiveRancanganResolusiBox({ isActive, proposals, mode = 'voting' }: ActiveRancanganResolusiBoxProps) {
  return (
    <div className={`rounded-3xl border overflow-hidden transition-all duration-500 ${
      isActive
        ? "border-cyan-500/50 bg-cyan-500/10 shadow-lg shadow-cyan-500/20"
        : proposals.length > 0
        ? "border-cyan-500/30 bg-cyan-500/5 shadow-none"
      ...
      <div className={`flex items-center gap-3 px-6 py-4 border-b ${
        ...
        <div className="flex flex-col">
          ...
          {proposals.length > 0 && (
            <span className="text-[9px] font-bold text-cyan-500/60 leading-none mt-0.5">
      ...
      {/* Content */}
      <div className="p-5 flex flex-col gap-3 min-h-[140px] max-h-[300px] overflow-y-auto scrollbar-thin scrollbar-thumb-cyan-500/20 scrollbar-track-transparent">
        {proposals.length > 0 ? (
          proposals.map((proposal) => (
            <div 
              key={proposal.id}
              className="p-4 rounded-2xl bg-zinc-900/60 border border-cyan-500/10 hover:border-cyan-500/30 transition-all space-y-3"
            >
              {/* Proposer to Target */}
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 min-w-0">
                  <div className="flex items-center gap-1.5 min-w-0">
                    <Globe className="h-3 w-3 text-cyan-500/50 shrink-0" />
                    <span className="text-[10px] font-black text-white truncate">{proposal.proposerCountry}</span>
                  </div>
                  {proposal.targetCountry && (
                    <>
                      <Target className="h-3 w-3 text-cyan-500/30 shrink-0" />
                      <div className="flex items-center gap-1.5 min-w-0">
                        <span className="text-[10px] font-black text-cyan-400 truncate">{proposal.targetCountry}</span>
                      </div>
                    </>
                  )}
                </div>
                ...
                <>
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between text-[8px] font-black uppercase tracking-tighter">
                      ...
                      <span className="text-cyan-400">{30 - proposal.daysRemaining}/30 Hari</span>
                    </div>
                  ...
                  {/* Countdown Footer */}
                  <div className="flex items-center justify-between pt-2 border-t border-cyan-500/5">
                ...
                <div className="flex items-center justify-between pt-2 border-t border-cyan-500/10">
                  ...
                  <span className="text-[9px] font-bold text-zinc-500">
                    ...
                    {proposal.implementationDaysRemaining !== undefined && (
                      <span className="text-cyan-400 ml-1">({proposal.implementationDaysRemaining} Hari)</span>
                    )}
                  </span>
                </div>
              )}
