...
export function ActiveRancanganResolusiBox({ isActive, proposals, mode = 'voting' }: ActiveRancanganResolusiBoxProps) {
  return (
    <div className={`rounded-3xl border overflow-hidden transition-all duration-500 ${
      ...
      {/* Content */}
      <div className="p-5 flex flex-col gap-3 min-h-[140px] max-h-[300px] overflow-y-auto scrollbar-thin scrollbar-thumb-cyan-900/50 scrollbar-track-transparent">
        {proposals && proposals.length > 0 ? (
          proposals.map((res: any, i: number) => (
            <div key={i} className="p-4 rounded-2xl bg-zinc-900/80 border border-cyan-500/20 flex flex-col gap-2 animate-in slide-in-from-right duration-300">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-black text-cyan-400 uppercase tracking-tight">{res.name}</span>
              ...
              <div className="flex items-center gap-2">
                 ...
                 <span className="text-[8px] text-emerald-500/80 font-black uppercase tracking-widest">Status: Terimplementasi</span>
              </div>
            </div>
          ))
        ) : (
