...
interface ReligionInfoOverlayProps {
  religion: string;
  effects: {
    plus: string[];
    minus: string[];
...
export function ReligionInfoOverlay({ religion, effects, onClose, onEffectClick }: ReligionInfoOverlayProps) {
  return (
    <div className="absolute inset-0 z-20 bg-zinc-950/98 backdrop-blur-xl p-4 flex flex-col animate-in fade-in zoom-in-95 duration-300">
      <div className="flex justify-between items-center mb-3">
        <h4 className="text-[9px] font-black text-amber-500 uppercase tracking-widest italic leading-none">{religion} Effects</h4>
      ...
      <button
        onClick={onClose}
        className="mt-3 w-full py-2 bg-amber-500 text-black rounded-lg text-[9px] font-black uppercase tracking-widest transition-all cursor-pointer shadow-lg active:scale-95"
      >
        Kembali
      </button>
    </div>
