...
import { Radio, Coins, Zap, CheckCircle2 } from "lucide-react";
import { unITUStorage } from "./unITUStorage";
...
export default function ITUMenu({ currentCash, currentDate, onUpdate }: OrgProps) {
    const [state, setState] = useState(unITUStorage.getData());

    useEffect(() => {
        setState(unITUStorage.getData());
    }, []);

    const handleJoin = () => {
        const result = unITUStorage.join(currentCash, currentDate);
        if (result.success) {
            setState(unITUStorage.getData());
    ...
    return (
        <div className="bg-zinc-950/20 border border-zinc-800/30 rounded-3xl p-6 space-y-6 backdrop-blur-md">
            <div className="flex items-center gap-3">
                ...
                <h3 className="text-lg font-black text-white uppercase italic">ITU Details</h3>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4 text-left font-sans not-italic">
                <div className="p-4 bg-zinc-900/40 border border-zinc-800/50 rounded-2xl">
                    <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-1.5 flex items-center gap-2">
                        <Zap size={12} className="text-violet-500" /> Dampak Bergabung
                    </p>
                    <p className="text-[11px] text-zinc-300 font-bold leading-relaxed">
                        Cyber Defense +15% & Riset Komunikasi ↑.
                    </p>
                </div>
