...
export default function OAS({ currentCash, currentDate, onUpdate }: OASProps) {
  const userCountryName = (localStorage.getItem("selectedCountry") || "Indonesia").toLowerCase();
  const isEligible = unMembershipStorage.isEligibleForRegional("oas", userCountryName);

  const handleJoin = () => {
    if (!isEligible) {
      alert("Permohonan Ditolak: Organisasi Negara-Negara Amerika (OAS) hanya menerima negara dari benua Amerika.");
      return;
    }

    unMembershipStorage.joinOrganization("oas");
    onUpdate();
  };
      ...
      <div className="bg-gradient-to-br from-purple-600/10 to-zinc-900/40 border border-purple-500/10 rounded-[32px] p-8 relative overflow-hidden group">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h3 className="text-2xl font-black text-white uppercase italic tracking-tight">OAS</h3>
              <p className="text-[10px] text-purple-400 font-bold uppercase tracking-[0.2em]">OAS</p>
            </div>
          </div>
          <p className="text-sm text-zinc-400 leading-relaxed max-w-2xl font-medium">
            Organisasi Negara-Negara Amerika untuk solidaritas regional.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-zinc-900/30 border border-zinc-800 rounded-2xl p-6 flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-lg font-black text-white italic">+20% Demokrasi</p>
          </div>
        </div>
        <div className="bg-zinc-900/30 border border-zinc-800 rounded-2xl p-6 flex items-center gap-6">
