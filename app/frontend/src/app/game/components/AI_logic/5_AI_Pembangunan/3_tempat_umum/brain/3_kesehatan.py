...
def decide():
    try:
        ...
        options = [o for o in input_data.get("options", []) if o.get("groupId") == "kesehatan"]
        if not options: return {"decision": "SKIP"}
        # Logic: If health index is low, build health facilities
        if metrics.get("health_index", 100) < 80:
