...
export default function SideMenu({ activeMenu, setActiveMenu, unreadCount }: SideMenuProps) {
  return (
    <div className="absolute left-8 top-1/4 z-50 flex flex-col items-center bg-zinc-950/40 backdrop-blur-2xl border border-white/5 rounded-3xl p-1.5 shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden animate-in slide-in-from-left-10 duration-700">
      ...
      <button
        onClick={() => setActiveMenu("Menu:Berita:all")}
        className={`relative group flex flex-col items-center justify-center w-16 h-24 rounded-2xl transition-all cursor-pointer overflow-hidden ${activeMenu.startsWith("Menu:Berita") ? 'bg-emerald-500/20 shadow-[inset_0_0_20px_rgba(16,185,129,0.2)]' : 'hover:bg-white/5'}`}
      >
        <Newspaper className={`h-6 w-6 ${activeMenu.startsWith("Menu:Berita") ? 'text-emerald-400 scale-110' : 'text-zinc-400'} group-hover:text-emerald-400 group-hover:scale-110 transition-all`} />
        <span className={`text-[10px] font-black uppercase tracking-[0.1em] mt-2 ${activeMenu.startsWith("Menu:Berita") ? 'text-emerald-400' : 'text-zinc-300'} group-hover:text-emerald-400 transition-colors`}>Update</span>
        <span className="text-[7px] font-bold text-zinc-500 uppercase tracking-widest leading-none mt-0.5">News</span>

        {/* Active Indicator */}
        {activeMenu.startsWith("Menu:Berita") && (
