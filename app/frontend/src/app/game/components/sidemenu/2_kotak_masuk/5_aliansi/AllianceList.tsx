...
export const AllianceList: React.FC<AllianceListProps> = ({
  ...
  const filteredMessages = messages
    .filter(msg => {
      if (msg.category === 'alliance') return true;
      if (msg.category === 'defense' && msg.subject.toLowerCase().includes('aliansi')) return true;
      return false;
    })
    .filter(msg => 
  ...
  if (filteredMessages.length === 0) {
    return (
      <div className="h-64 flex flex-col items-center justify-center text-zinc-600">
        ...
        <p className="text-xs font-black uppercase tracking-[0.3em]">Tidak Ada Pesan Aliansi Global</p>
      </div>
    );
  }
