...
export const PactList: React.FC<PactListProps> = ({
  messages,
  expandedId,
  setExpandedId,
  handleAction,
  ...
  const filteredMessages = messages
    .filter(msg => {
      if (msg.category === 'pact') return true;
      if (msg.category === 'defense' && msg.subject.toLowerCase().includes('pakta')) return true;
      return false;
    })
    .filter(msg => 
  ...
  if (filteredMessages.length === 0) {
    return (
      <div className="h-64 flex flex-col items-center justify-center text-zinc-600">
        ...
        <p className="text-xs font-black uppercase tracking-[0.3em]">Tidak Ada Pesan Pakta Pertahanan</p>
      </div>
    );
  }
